...
export const FolderBrowser: React.FC<FolderBrowserProps> = ({
  ...
  const shareItem = async (item: FileItem | FolderItem, path: string) => {
    ...
    const url = `${window.location.origin}?target=${item.id}&path=${[...pathSegments, toSlug(item.name)].join('/')}`;
    
    const text = `📑 ${item.type === 'folder' ? 'Folder' : 'File'}:  
${item.name}  

🔗 ${url}  

📤 Shared via ${window.location.host}`;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 flex-shrink-0">
            <Button
              variant="outline"
              size="sm"
              onClick={() => currentFolder && toggleBookmark(currentFolder, currentPath)}
              className="hover:bg-accent/50 transition-colors"
            >
              <Star 
                className={`w-4 h-4 mr-1 transition-colors ${
                  currentFolder && isBookmarked(currentFolder.id)
                    ? 'fill-yellow-400 text-yellow-400'
                    : 'text-muted-foreground'
                }`}
              />
              <span className="hidden sm:inline">
                {currentFolder && isBookmarked(currentFolder.id) ? 'Bookmarked' : 'Bookmark'}
              </span>
            </Button>
          </div>
        </div>
                        ...
                        <div className="flex items-start justify-between">
                          ...
                          <div className="flex items-center space-x-1">
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                toggleBookmark(folder, currentPath);
                              }}
                          ...
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex items-center space-x-1">
                              <Button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  toggleBookmark(file, currentPath);
                                }}
                                className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity p-1 h-auto hover:bg-accent/50"
                              >
                                <Bookmark
