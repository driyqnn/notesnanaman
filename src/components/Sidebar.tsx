...
const BookmarkManagerLazy = lazy(() => import('./BookmarkManager').then(m => ({ default: m.BookmarkManager })));
      ...
      `}>
        ...
        <ScrollArea id="sidebar-content" aria-label="Sidebar content" className="flex-1">
          <div className="p-4 space-y-4">
            ...
            {/* Bookmarks */}
            <Suspense fallback={<div className="text-xs text-muted-foreground">Loading bookmarks…</div>}>
              <BookmarkManagerLazy
                bookmarks={bookmarks}
                onRemoveBookmark={removeBookmark}
                onClearAll={clearAllBookmarks}
                onNavigateToItem={(bookmark) => {
                  if (bookmark.type === "folder" && onNavigateToFolder) {
                    onNavigateToFolder(bookmark.id, bookmark.name);
                  } else if (bookmark.type === "file" && onNavigateToBookmark) {
