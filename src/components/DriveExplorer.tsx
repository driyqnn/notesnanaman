...
export const DriveExplorer: React.FC<DriveExplorerProps> = ({ targetFileId, initialPath, initialFolderId }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (dataResponse.ok && changelogResponse.ok) {
        ...
        if (hasNewVersion) {
          console.log('📊 New version detected:', driveData.version);
        }
  ...
  useEffect(() => {
    ...
    if (result) {
      ...
      // Remove target param after a delay to allow the file preview to load and UI to become responsive
      // This ensures shared links work properly while keeping the UI functional
      ...
      }, 2000); // 2 seconds delay to allow file preview to load
  ...
  // Handle bookmark navigation for files
  const handleBookmarkNavigation = useCallback((bookmark: BookmarkItem) => {
